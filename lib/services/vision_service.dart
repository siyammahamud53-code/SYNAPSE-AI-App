...
class VisionService extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    ...
    try {
      VisionLogger.info('Initializing VisionService...');
      await _initializeCamera();
      await _initializeDetectors();
      
      _isInitialized = true;
      VisionLogger.info('VisionService initialized successfully');
      notifyListeners();
    } catch (e, stackTrace) {
      VisionLogger.error('VisionService initialization failed: $e', stackTrace);
      rethrow;
    }
  }
  ...
  Future<void> _initializeCamera() async {
    try {
      ...
      if (_cameras != null && _cameras!.isNotEmpty) {
        ...
        VisionLogger.info('Camera initialized: ${camera.name}');
      } else {
        VisionLogger.warning('No cameras available');
      }
    } catch (e) {
      VisionLogger.error('Camera initialization failed: $e');
  ...
  Future<void> _initializeDetectors() async {
    try {
      ...
      VisionLogger.info('ML Kit detectors initialized');
    } catch (e) {
      VisionLogger.error('Detectors initialization failed: $e');
    }
  }
  ...
  Future<String?> captureImage() async {
    try {
      if (_cameraController == null || !_cameraController!.value.isInitialized) {
        VisionLogger.warning('Camera not available, using image picker');
        return await _pickImageFromGallery();
      }
      
      final XFile? image = await _cameraController!.takePicture();
      if (image != null) {
        VisionLogger.info('Image captured: ${image.path}');
        return image.path;
      }
      
      return null;
    } catch (e) {
      VisionLogger.error('Image capture failed: $e');
      return null;
    }
  }
  ...
  Future<String?> _pickImageFromGallery() async {
    try {
      ...
      if (image != null) {
        VisionLogger.info('Image picked: ${image.path}');
        return image.path;
      }
      
      return null;
    } catch (e) {
      VisionLogger.error('Image pick failed: $e');
      return null;
    }
  }
  ...
  Future<Map<String, dynamic>> processImage(String imagePath) async {
    ...
    try {
      ...
      try {
        ...
        VisionLogger.error('Face detection failed: $e');
        results['faces'] = [];
      }
      ...
      try {
        ...
        VisionLogger.error('Object detection failed: $e');
        results['objects'] = [];
      }
      ...
      try {
        ...
        VisionLogger.error('Text recognition failed: $e');
        results['text'] = '';
        results['textBlocks'] = [];
      }
      ...
      try {
        ...
        VisionLogger.error('Pose detection failed: $e');
        results['poses'] = [];
      }
      ...
      VisionLogger.info('Image processed: ${results.keys.join(', ')}');
      return results;
    } catch (e, stackTrace) {
      VisionLogger.error('Image processing failed: $e', stackTrace);
      rethrow;
    } finally {
      _isProcessing = false;
      notifyListeners();
  ...
  Future<String?> takeScreenshot() async {
    try {
      ...
      if (result != null) {
        VisionLogger.info('Screenshot taken');
        return result.toString();
      }
      return null;
    } catch (e) {
      VisionLogger.error('Screenshot failed: $e');
      return null;
    }
  }
